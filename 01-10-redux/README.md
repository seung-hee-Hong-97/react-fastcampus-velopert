...
-   클래스형 컴포넌트에서 리덕스를 연동하는 방법에 관한 내용
-   😁 거의 쓸 일이 없고 클래스형 컴포넌트보다는 함수형 컴포넌트 학습에 집중하고 있으므로 해당 강좌에 관해서는 필요할 때 학습하기로 하였음.

## 정리

-   리덕스는 어려울 수 있으므로 진입장벽이 있다.
-   리덕스 사용에 적응이 되고 프로젝트 규모가 커질수록 정말 편해진다는 것을 배우게 될 것이다.
-   취향이 아니라면 MobX나 Context API를 활용하면 된다.
-   Context API의 Global 상태와 별반 다를 게 없다. (개발 흐름이 유사하다.)
    -   다만 리덕스에는 유용한 함수가 들어있고 훅도 들어있다.
    -   리덕스로 관리하는 것이 Context API를 사용하는 것보다 관리하기가 굉장히 수월하다.
    -   그러므로 대규모 프로젝트에서는 리덕스를 사용하는 것을 권장한다.
-   다음 튜토리얼은 리덕스 미들웨어에 대해 배우는데, 리덕스 미들웨어와 함께라면 일반 Context API와 큰 차이점이 발생한다. (이어서 학습할 예정임)
